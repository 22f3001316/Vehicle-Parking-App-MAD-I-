...
def user_dashboard():
    ...
    return render_template('user_dashboard.html', user=current_user, lots=lots, reservations=reservations)
...
def user_book_spot(lot_id):
    ...
    return render_template('book_spot.html', lot_id=lot_id, spot_id=available_spot.id)
...
def user_release_spot(spot_id):
    ...
    if request.method == 'POST' and spot.status == 'O':
        ...
        time_hrs = (reservation.leaving_timestamp - reservation.parking_timestamp).total_seconds() / 3600
        cost = round(time_hrs * lot.price, 2)
    ...
    est_cost = round(((est_time - reservation.parking_timestamp).total_seconds() / 3600) * lot.price, 2)
...
def user_search():
    ...
    return render_template('user_dashboard.html', lots=lots, username=session['username'])
...
def user_summary():
    ...
    for lot in lots:
        ...
        occupancy.append({'name': lot.prime_location_name, 'available': available, 'occupied': occupied})
        revenues.append({'name': lot.prime_location_name, 'revenue': revenue})

    return render_template('user_summary.html', occupancy=occupancy, revenues=revenues)
